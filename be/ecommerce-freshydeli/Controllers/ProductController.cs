    ...
    public class ProductController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteProductByProductId(int productId)
        {
            ...
            if (orderDetails.Count == 0)
            {
                ...
                List<Product> productList = await Context.Product.AsNoTracking().Where(p => p.CompanyId == companyId).Include(p => p.Currency).Include(p => p.Iva).Include(p => p.Category).ToListAsync();
            ...
            List<Product> products = await Context.Product.AsNoTracking().Where(p => p.CompanyId == companyId).Include(p => p.Currency).Include(p => p.Iva).Include(p => p.Category).ToListAsync();
       ...
       public async Task<IActionResult> updateProduct([FromBody] ProductDTO productDTO)
       {
        ...
        [HttpPost("createProductByRange")]
        public async Task<IActionResult> CreateProductByRange([FromBody] List<ProductDTO> productDTO)
        {
           List< Product> product = Mapper.Map<List<Product>>(productDTO);

            //          PriceListProduct priceListProduct = Mapper.Map<PriceListProduct>(productDTO);

            await Context.AddRangeAsync(product);
        ...
        {
            try { 
            List<Product> products = await Context.Product.AsNoTracking().Where(i => i.IvaId == id).ToListAsync();
